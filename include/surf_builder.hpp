...
public:
    ...
    explicit SuRFBuilder(
        bool include_dense, uint32_t sparse_dense_ratio, SuffixType suffix_type, level_t hash_suffix_len, level_t real_suffix_len)
    ...
    inline bool hasKeys() const { return has_keys_; }
    ...
    inline level_t getTreeHeight() const { return static_cast<level_t>(labels_.size()); }
    ...
    const std::vector<std::vector<word_t>> & getSuffixes() const { return suffixes_; }
    const std::vector<position_t> & getSuffixCounts() const { return suffix_counts_; }
    const std::vector<position_t> & getNodeCounts() const { return node_counts_; }
    inline level_t getSparseStartLevel() const { return sparse_start_level_; }
    inline SuffixType getSuffixType() const { return suffix_type_; }
    inline level_t getSuffixLen() const { return hash_suffix_len_ + real_suffix_len_; }
    inline level_t getHashSuffixLen() const { return hash_suffix_len_; }
    inline level_t getRealSuffixLen() const { return real_suffix_len_; }

private:
    static bool isSameKey(const std::string & a, const std::string & b) { return a.compare(b) == 0; }
...
inline void SuRFBuilder::build(const std::vector<std::string> & keys)
{
    assert(keys.size() > 0);
    buildSparse(keys);
    if (include_dense_)
...
inline void SuRFBuilder::buildSparse(const std::vector<std::string> & keys)
{
...
inline level_t SuRFBuilder::skipCommonPrefix(const std::string & key)
{
    level_t level = 0;
...
inline level_t
SuRFBuilder::insertKeyBytesToTrieUntilUnique(const std::string & key, const std::string & next_key, const level_t start_level)
{
...
inline void SuRFBuilder::insertKeyByte(const char c, const level_t level, const bool is_start_of_node, const bool is_term)
{
...
inline void SuRFBuilder::buildDense()
{
...
inline void SuRFBuilder::initDenseVectors(const level_t level)
{
...
inline void SuRFBuilder::setLabelAndChildIndicatorBitmap(const level_t level, const position_t node_num, const position_t pos)
{
...
inline void SuRFBuilder::addLevel()
{
...
inline position_t SuRFBuilder::getNumItems(const level_t level) const
{
    return static_cast<position_t>(labels_[level].size());
}

inline bool SuRFBuilder::isStartOfNode(const level_t level, const position_t pos) const
{
    return readBit(louds_bits_[level], pos);
}

inline bool SuRFBuilder::isTerminator(const level_t level, const position_t pos) const
{
    label_t label = labels_[level][pos];
    return ((label == kTerminator) && !readBit(child_indicator_bits_[level], pos));
}

inline bool SuRFBuilder::insert(const std::string & key)
{
...
inline void SuRFBuilder::finalize()
{
