...
public:
    ...
    public:
        ...
        inline bool isSearchComplete() const { return is_search_complete_; }
        inline bool isMoveLeftComplete() const { return is_move_left_complete_; }
        inline bool isMoveRightComplete() const { return is_move_right_complete_; }
        inline bool isComplete() const { return (is_search_complete_ && (is_move_left_complete_ && is_move_right_complete_)); }
        ...
        inline position_t getSendOutNodeNum() const { return send_out_node_num_; }
    ...
    private:
        ...
        inline void
        setFlags(const bool is_valid, const bool is_search_complete, const bool is_move_left_complete, const bool is_move_right_complete);
...
public:
    ...
    inline bool moveToKeyGreaterThan(const std::string & key, const bool inclusive, LoudsDense::Iter & iter) const;
    inline uint64_t approxCount(
        const LoudsDense::Iter * iter_left,
        const LoudsDense::Iter * iter_right,
        position_t & out_node_num_left,
        position_t & out_node_num_right) const;

    inline uint64_t getHeight() const { return height_; }
    ...
    static LoudsDense * deSerialize(char *& src)
    {
        ...
        memcpy(louds_dense->level_cuts_, src, sizeof(position_t) * (louds_dense->height_));
...
private:
    ...
    inline bool compareSuffixGreaterThan(const position_t pos, const std::string & key, const level_t level, LoudsDense::Iter & iter) const;
    inline void extendPosList(std::vector<position_t> & pos_list, position_t & out_node_num) const;
...
inline LoudsDense::LoudsDense(const SuRFBuilder * builder)
{
    ...
    child_indicator_bitmaps_
        = new BitvectorRank(kRankBasicBlockSize, builder->getBitmapChildIndicatorBits(), num_bits_per_level, 0, height_);
    prefixkey_indicator_bits_
        = new BitvectorRank(kRankBasicBlockSize, builder->getPrefixkeyIndicatorBits(), builder->getNodeCounts(), 0, height_);
    ...
    else
    {
        ...
        suffixes_ = new BitvectorSuffix(
            builder->getSuffixType(), hash_suffix_len, real_suffix_len, builder->getSuffixes(), num_suffix_bits_per_level, 0, height_);
    }
}

inline bool LoudsDense::lookupKey(const std::string & key, position_t & out_node_num) const
{
...
inline bool LoudsDense::moveToKeyGreaterThan(const std::string & key, const bool inclusive, LoudsDense::Iter & iter) const
{
    (void)inclusive;
    position_t node_num = 0;
    position_t pos = 0;
...
inline void LoudsDense::extendPosList(std::vector<position_t> & pos_list, position_t & out_node_num) const
{
    position_t node_num = 0;
...
inline uint64_t LoudsDense::approxCount(
    const LoudsDense::Iter * iter_left,
    const LoudsDense::Iter * iter_right,
    position_t & out_node_num_left,
    position_t & out_node_num_right) const
{
    ...
    for (level_t i = 0; i < height_; i++)
    {
        ...
        if (left_pos < right_pos)
        {
            ...
            bool has_prefix_key_left = prefixkey_indicator_bits_->readBit(left_pos / kNodeFanout);
            bool has_prefix_key_right = prefixkey_indicator_bits_->readBit(right_pos / kNodeFanout);
            ...
            position_t rank_left_prefix = prefixkey_indicator_bits_->rank(left_pos / kNodeFanout);
            position_t rank_right_prefix = prefixkey_indicator_bits_->rank(right_pos / kNodeFanout);
            position_t num_leafs
                = (rank_right_label - rank_left_label) - (rank_right_ind - rank_left_ind) + (rank_right_prefix - rank_left_prefix);
...
inline uint64_t LoudsDense::serializedSize() const
{
    uint64_t size = sizeof(height_) + (sizeof(position_t) * height_);
    sizeAlign(size);
    size
        += (label_bitmaps_->serializedSize() + child_indicator_bitmaps_->serializedSize() + prefixkey_indicator_bits_->serializedSize()
            + suffixes_->serializedSize());
    sizeAlign(size);
    return size;
}

inline uint64_t LoudsDense::getMemoryUsage() const
{
    return (
        sizeof(LoudsDense) + label_bitmaps_->size() + child_indicator_bitmaps_->size() + prefixkey_indicator_bits_->size()
        + suffixes_->size());
}
...
inline position_t LoudsDense::getSuffixPos(const position_t pos, const bool is_prefix_key) const
{
    ...
    position_t suffix_pos
        = (label_bitmaps_->rank(pos) - child_indicator_bitmaps_->rank(pos) + prefixkey_indicator_bits_->rank(node_num) - 1);
...
inline position_t LoudsDense::getPrevPos(const position_t pos, bool * is_out_of_bound) const
{
...
inline bool
LoudsDense::compareSuffixGreaterThan(const position_t pos, const std::string & key, const level_t level, LoudsDense::Iter & iter) const
{
...
inline void LoudsDense::Iter::clear()
{
    is_valid_ = false;
    key_len_ = 0;
    is_at_prefix_key_ = false;
...
inline int LoudsDense::Iter::compare(const std::string & key) const
{
...
inline std::string LoudsDense::Iter::getKey() const
{
    if (!is_valid_)
    {
        return std::string();
...
inline int LoudsDense::Iter::getSuffix(word_t * suffix) const
{
...
inline std::string LoudsDense::Iter::getKeyWithSuffix(unsigned * bitlen) const
{
    std::string iter_key = getKey();
    if (isComplete() && ((trie_->suffixes_->getType() == kReal) || (trie_->suffixes_->getType() == kMixed)))
    {
...
inline void LoudsDense::Iter::setFlags(
    const bool is_valid, const bool is_search_complete, const bool is_move_left_complete, const bool is_move_right_complete)
{
    is_valid_ = is_valid;
...
inline void LoudsDense::Iter::setToFirstLabelInRoot()
{
...
inline void LoudsDense::Iter::setToLastLabelInRoot()
{
...
inline void LoudsDense::Iter::moveToLeftMostKey()
{
    assert(key_len_ > 0);
...
inline void LoudsDense::Iter::moveToRightMostKey()
{
    assert(key_len_ > 0);
    level_t level = key_len_ - 1;
    position_t pos = pos_in_trie_[level];
...
inline void LoudsDense::Iter::operator++(int)
{
    assert(key_len_ > 0);
    if (is_at_prefix_key_)
    {
